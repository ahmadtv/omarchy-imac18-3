...
| 🎧 **Apple EarPods** | Fully supported. Plug them in and audio + mic follow the jack; unplug and it's back to internal — automatically. All three **inline buttons** work too: tap for play/pause, and hold volume up or down to ramp the volume like a held key. Holding the centre button sends the assistant key (`KEY_VOICECOMMAND`), free to bind to dictation. |
...
- 🎙️ **EarPods recordings start with a burst** — when the audio chip has been idle a while, the first ~0.3 s of a headset-mic recording holds a loud spike (the chip powering up). Your ears are safe: the earpieces are no longer powered at recording start, so nothing is played back at you. Being worked on.
...
| [drm/i915#17042](https://gitlab.freedesktop.org/drm/i915/kernel/-/issues/17042) | Hidden Intel HD 630 running headless: a quirk so i915 creates no outputs on iMacs, then the kernel's set_os list gains `iMac18,3` | Open; Intel replied 2026-09-16 pointing at `i915.disable_display=1` |
...
| [omacom/omarchy#10985](https://github.com/omacom/omarchy/pull/10985) | Omarchy's audio panel lists each jack's ports as rows, the way macOS and GNOME do | Closed 2026-09-21 as a duplicate of [#7205](https://github.com/omacom/omarchy/pull/7205), which stays open |
