...
- **Fix: rebuilding the brightness table on a machine already running the 1..100 table failed** with `invalid literal for int() ... 'One'`. The ACPI disassembler writes level 1 as the word `One`, not `0x01`, and the generator only understood numbers. It now reads both, so `sudo igpu/linux-side/acpi/make-bcl100` works from any earlier version of the table.
